...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		createImgUploadControls: function(){
			...
			var adjustUploadButton = '';
			
			if(that.options.uploadData.imageType == 4){
				adjustUploadButton = 'cropControls2';
			}
			
			if(that.options.customUploadButtonId ===''){ cropControlUpload = '<i class="cropControlUpload" title="Carregar Foto"></i>'; }
			var cropControlRemoveCroppedImage = '<i class="cropControlRemoveCroppedImage" title="Remover Foto"></i>';
			...
			var html =    '<div class="cropControls cropControlsUpload ' + adjustUploadButton + '"> ' + cropControlUpload + cropControlRemoveCroppedImage + ' </div>';
		...
		afterUpload: function(data){
            ...
            if (response.status == 'success') {
                ...
                {
                	var imgThumb = response.urlThumb;
                	var imgLarge = response.urlLarge;
                	
                	$('.addphoto').remove();
                	                	
                	var htmlDelete = '<div class="cropControls cropControlsUpload" onclick="removeImg(\'' + response.hash + '\')"><i class="cropControlRemoveCroppedImage" title="Remover Foto"></i></div>';
        			                	
			    	var $newItems = $('<div class="grid-item transition metal ium hash_' + response.hash + '" style="background: url(\'' + imgThumb + '\') no-repeat;">' +			                    					    							
			    							htmlDelete +            	
			    							'<div class="grid_hover_area text-center">' +
						                    '<div class="grid_hover_text m-top-110">' +
						                    	'<h4 class="showphoto"><input type="checkbox" value="1"> <i class="fa fa-eye"></i></h4>' +
						                        '<input type="text" maxlength="30" placeholder="Informe um titulo para sua foto" id="title" style="width: 300px; text-align: center; color: #ccc; background: transparent; border: 1px solid #ccc;"></p>' +
						                        '<input type="text" placeholder="Descreva sua foto" id="description" style="width: 300px; text-align: center; color: #ccc; background: transparent; border: 1px solid #ccc;"></p>' +
						                        '<a href="' + imgLarge + '" class="popup-img text-white m-top-20">Ampliar <i class="fa fa-search-plus"></i></a>' +
						                    '</div>' +
						                '</div>' +		
						        	'</div>' +
										...
										'<div id="imgGalleryModel"></div>' +
						            	 '<div class="grid_hover_area2 text-center">' +
						                    '<div class="grid_hover_text m-top-110">' +
												...
												'<h6 class="text-white">Adicionar mais fotos</h6>' +
						                    '</div>' +
						                '</div>' +
						        	'</div>');
			    	...
			    	var cropGalleryOptions = {
							uploadUrl: that.options.uploadUrl,						
							uploadData:{						
								"personAlias": response.personAlias,
								"apid": response.apid,
								"imageType": 2,
								"thumbWidth": 370,
								"thumbHeight": 278,
					...
					var cropGallery = new Croppic('imgGalleryModel', cropGalleryOptions);
			    	
                }else{
                	
                	var imgThumb = $('<img src="'+response.urlThumb+'" style="width:'+ response.width +'px; height:'+ response.height +'px;">');                	
                	that.obj.append(imgThumb);
                }
		...
		createCropControls: function(){
	        ...
	        var cropControlCrop =            '<i class="cropControlCrop" title="Cortar"></i>';
			var cropControlReset =           '<i class="cropControlReset" title="Cancelar"></i>';
